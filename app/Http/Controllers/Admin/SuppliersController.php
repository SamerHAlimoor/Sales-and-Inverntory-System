...
class SuppliersController extends Controller
{
    ...
    public function index()
    {
        ...
        $data = get_cols_where_p(new Suppliers(), array("*"), array("com_code" => $com_code), 'id', 'DESC', PAGINATION_COUNT);
        // return $data;
        if (!empty($data)) {
    ...
    {
        $com_code = auth()->user()->com_code;
        $suppliers_categories = get_cols_where(new SupplierCategories(), array('id', 'name'), array('com_code' => $com_code, 'active' => 0), 'id', 'DESC');
    ...
    public function store(SupplierRequest $request)
    {
        ...
        try {
            ...
            $checkExists_name = get_cols_where_row(new Suppliers(), array("id"), array('name' => $request->name, 'com_code' => $com_code));
            ...
            //set customer code
            $row = get_cols_where_row_orderby(new Suppliers(), array("supplier_code"), array("com_code" => $com_code), 'id', 'DESC');
            if (!empty($row)) {
            ...
            if ($flag) {
                ...
                $suppliers_parent_account_number = get_field_value(new AdminPannelSetting(), "suppliers_parent_account_number", array('com_code' => $com_code));
    ...
    {
        $com_code = auth()->user()->com_code;
        $data = get_cols_where_row(new Suppliers(), array("*"), array("id" => $id, "com_code" => $com_code));
        $suppliers_categories = get_cols_where(new SupplierCategories(), array('id', 'name'), array('com_code' => $com_code, 'active' => 0), 'id', 'DESC');
    ...
    public function update($id, SupplierUpdateRequest $request)
    {
        try {
            $com_code = auth()->user()->com_code;
            $data = get_cols_where_row(new Suppliers(), array("id", "account_number", "supplier_code"), array("id" => $id, "com_code" => $com_code));
            if (empty($data)) {
                return redirect()->route('admin.supplier.index')->with(['error' => 'عفوا غير قادر علي الوصول الي البيانات المطلوبة !!']);
            }
            $checkExists = Suppliers::where(['name' => $request->name, 'com_code' => $com_code])->where('id', '!=', $id)->first();
            if ($checkExists != null) {
                return redirect()->back()
            ...
            $flag = update(new Suppliers(), $data_to_update, array('id' => $id, 'com_code' => $com_code));
            if ($flag) {
    ...
    public function delete($id)
    {
        try {
            $com_code = auth()->user()->com_code;
            $item_row = get_cols_where_row(new Suppliers(), array("name"), array("id" => $id, "com_code" => $com_code));
            if (!empty($item_row)) {
                $flag = delete(new Suppliers(), ["id" => $id, "com_code" => $com_code]);
                if ($flag) {
                    return redirect()->back()
    ...
    public function ajax_search(Request $request)
    {
        if ($request->ajax()) {
            ...
            $data = Suppliers::where($field1, $operator1, $value1)->where($field2, $operator2, $value2)->where($field3, $operator3, $value3)->where(['com_code' => $com_code])->orderBy('id', 'DESC')->paginate(PAGINATION_COUNT);
            if (!empty($data)) {
                foreach ($data as $info) {
                    $info->added_by_admin = Admin::where('id', $info->added_by)->value('name');
                    $info->suppliers_categories_name = SupplierCategories::where('id', $info->suppliers_categories_id)->value('name');
            ...
            //General mirror 
            $mirror['credit_sum'] = Suppliers::where($field1, $operator1, $value1)->where($field2, $operator2, $value2)->where("current_balance", "<", 0)->where($field3, $operator3, $value3)->where(['com_code' => $com_code])->sum('current_balance');
            $mirror['debit_sum'] = Suppliers::where($field1, $operator1, $value1)->where($field2, $operator2, $value2)->where("current_balance", ">", 0)->where($field3, $operator3, $value3)->where(['com_code' => $com_code])->sum('current_balance');
