...
class AccountsController extends Controller
{
    ...
    public function index()
    {
        ...
        if (!empty($data)) {
            foreach ($data as $info) {
                ...
                $info->related_internal_accounts = AccountType::where('id', $info->account_type)->value('related_internal_accounts');
                if ($info->is_parent == 0) {
        ...
        $account_types = get_cols_where(new AccountType(), array("id", "name"), array("active" => 0), 'id', 'ASC');
        return view('admin.accounts.index', ['data' => $data, 'account_types' => $account_types]);
    }
    public function create()
    {
        $com_code = auth()->user()->com_code;
        $account_types = get_cols_where(new AccountType(), array("id", "name"), array("active" => 0, "related_internal_accounts" => 1), 'id', 'ASC');
    ...
    public function edit($id)
    {
        ...
        $relatediternalaccounts = AccountType::where('id', $data['account_type'])->value('related_internal_accounts');
        if ($relatediternalaccounts == 0) {
            return redirect()->route('admin.accounts.index')->with(['error' => 'عفوا لايمكن تحديث هذا الحساب الإ من شاشته الخاصه حسب نوعه']);
        }
        $account_types = get_cols_where(new AccountType(), array("id", "name"), array("active" => 0), 'id', 'ASC');
        $parent_accounts = get_cols_where(new Account(), array("account_number", "name"), array("is_parent" => 1, "com_code" => $com_code), 'id', 'ASC');
        ...
        if ($request->ajax()) {
            ...
            if (!empty($data)) {
                foreach ($data as $info) {
                    ...
                    $info->account_types_name = AccountType::where('id', $info->account_type)->value('name');
                    $info->relatediternalaccounts = AccountType::where('id', $info->account_type)->value('related_internal_accounts');
                    if ($info->is_parent == 0) {
                        $info->parent_account_name = Account::where('account_number', $info->parent_account_number)->value('name');
                    } else {
                        $info->parent_account_name = "لايوجد";
