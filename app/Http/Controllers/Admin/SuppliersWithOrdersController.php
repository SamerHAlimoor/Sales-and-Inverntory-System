...
use App\Models\SuppliersWithOrdersDetails;
...
class SuppliersWithOrdersController extends Controller
{
    public function index()
    {
        ...
        $data = get_cols_where_p(new SuppliersWithOrders(), array("*"), array("com_code" => $com_code, 'order_type' => 1), 'id', 'DESC', PAGINATION_COUNT);
        if (!empty($data)) {
            foreach ($data as $info) {
                $info->added_by_admin = Admin::where('id', $info->added_by)->value('name');
                $info->supplier_name = Suppliers::where('supplier_code', $info->supplier_code)->value('name');
        ...
        $suppliers = get_cols_where(new Suppliers(), array('supplier_code', 'name'), array('com_code' => $com_code), 'id', 'DESC');
        $stores = get_cols_where(new Store(), array('id', 'name'), array('com_code' => $com_code, 'active' => 0), 'id', 'DESC');
        return view('admin.suppliers_with_orders.index', ['data' => $data, 'suppliers' => $suppliers, 'stores' => $stores]);
    }
    public function create()
    {
        $com_code = auth()->user()->com_code;
        $suppliers = get_cols_where(new Suppliers(), array('supplier_code', 'name'), array('com_code' => $com_code, 'active' => 1), 'id', 'DESC');
        $stores = get_cols_where(new Store(), array('id', 'name'), array('com_code' => $com_code, 'active' => 0), 'id', 'DESC');
        //return $suppliers;
    ...
    public function store(SuppliersWithOrdersRequest $request)
    {
        try {
            $com_code = auth()->user()->com_code;
            $supplierData = get_cols_where_row(new Suppliers(), array("account_number"), array("supplier_code" => $request->supplier_code, "com_code" => $com_code));
            if (empty($supplierData)) {
                return redirect()->back()
                    ->with(['error' => 'عفوا   غير قادر علي الوصول الي بيانات المورد المحدد'])
                    ->withInput();
            }
            $row = get_cols_where_row_orderby(new SuppliersWithOrders(), array("auto_serial"), array("com_code" => $com_code, 'order_type' => 1), 'id', 'DESC');
            if (!empty($row)) {
            ...
            SuppliersWithOrders::create($data_insert);
            $id = get_field_value(new SuppliersWithOrders(), "id", array("auto_serial" => $data_insert['auto_serial'], "com_code" => $com_code, "order_type" => 1));
    ...
    {
        // $com_code = auth()->user()->com_code;
        // $item_cards = get_cols_where(new InventoryItemCart(), array('id', 'name'), array("com_code" => $com_code, 'active' => 0), 'id', 'DESC');
        // return $item_cards;
        try {
            $com_code = auth()->user()->com_code;
            $data = get_cols_where_row(new SuppliersWithOrders(), array("*"), array("id" => $id, "com_code" => $com_code, 'order_type' => 1));
            if (empty($data)) {
                return redirect()->route('admin.suppliers_orders.index')->with(['error' => 'عفوا غير قادر علي الوصول الي البيانات المطلوبة !!']);
            }
            $data['added_by_admin'] = Admin::where('id', $data['added_by'])->value('name');
            $data['supplier_name'] = Suppliers::where('supplier_code', $data['supplier_code'])->value('name');
            $data['store_name'] = Store::where('id', $data['store_id'])->value('name');
            if ($data['updated_by'] > 0 and $data['updated_by'] != null) {
                $data['updated_by_admin'] = Admin::where('id', $data['updated_by'])->value('name');
            }
            $details = get_cols_where(new SuppliersWithOrdersDetails(), array("*"), array('suppliers_with_orders_auto_serial' => $data['auto_serial'], 'order_type' => 1, 'com_code' => $com_code), 'id', 'DESC');
            if (!empty($details)) {
                foreach ($details as $info) {
                    $info->item_card_name = InventoryItemCart::where('item_code', $info->item_code)->value('name');
            ...
            // $item_cards = get_cols_where_p(new InventoryItemCart(), array("*"), array("com_code" => $com_code, 'active' => 0), 'id', 'DESC', PAGINATION_COUNT);

            // $item_cards = SuppliersWithOrders::all();


            if ($data['is_approved'] == 0) {
                $item_card = get_cols_where(new InventoryItemCart(), array('id', 'name', 'item_type'), array("com_code" => $com_code, 'active' => 0), 'id', 'DESC');
            } else {
                $item_card = array();
            }
            // return $item_card;
            return view("admin.suppliers_with_orders.show", ['data' => $data, 'details' => $details, 'item_cards' => $item_card]);
        } catch (\Exception $ex) {
            return redirect()->back()
                ->with(['error' => 'عفوا حدث خطأ ما' . $ex->getMessage()]);
        }
    ...
    public function update($id, SuppliersWithOrdersRequest $request)
    {
        ...
        try {
            $com_code = auth()->user()->com_code;
            $data = get_cols_where_row(new SuppliersWithOrders(), array("is_approved"), array("id" => $id, "com_code" => $com_code, 'order_type' => 1));
            if (empty($data)) {
            ...
            update(new SuppliersWithOrders(), $data_to_update, array("id" => $id, "com_code" => $com_code, 'order_type' => 1));
    ...
    public function get_item_uoms(Request $request)
    {
        if ($request->ajax()) {
            ...
            $item_card_Data = get_cols_where_row(new InventoryItemCart(), array("does_has_retail_unit", "retail_uom", "uom_id"), array("item_code" => $item_code, "com_code" => $com_code));
            if (!empty($item_card_Data)) {
                if ($item_card_Data['does_has_retail_unit'] == 1) {
                    $item_card_Data['parent_uom_name'] = get_field_value(new InvUoms(), "name", array("id" => $item_card_Data['uom_id']));
                    $item_card_Data['retail_uom_name'] = get_field_value(new InvUoms(), "name", array("id" => $item_card_Data['retail_uom_id']));
                } else {
                    $item_card_Data['parent_uom_name'] = get_field_value(new InvUoms(), "name", array("id" => $item_card_Data['uom_id']));
                }
            }
            // return dd($item_card_Data);
    ...
    public function addNewDetails(Request $request)
    {
        if ($request->ajax()) {
            ...
            $suppliers_with_ordersData = get_cols_where_row(
                new SuppliersWithOrders(),
                array("is_approved", "order_date", "tax_value", "discount_value"),
                array("auto_serial" => $request->auto_serial_parent, "com_code" => $com_code, 'order_type' => 1)
            );
            if (!empty($suppliers_with_ordersData)) {
                if ($suppliers_with_ordersData['is_approved'] == 0) {
                    $data_insert['suppliers_with_orders_auto_serial'] = $request->auto_serial_parent;
                    $data_insert['order_type'] = 1;
                    $data_insert['item_code'] = $request->item_code_add;
                    $data_insert['delivered_quantity'] = $request->qty_add;
                    $data_insert['unit_price'] = $request->price_add;
                    $data_insert['uom_id'] = $request->uom_id_Add;
                    $data_insert['is_parent_uom'] = $request->is_parent_uom;
                    if ($request->type == 2) {
                        $data_insert['production_date'] = $request->production_date;
                        $data_insert['expire_date'] = $request->expiration_date;
                    }
                    ...
                    $data_insert['com_code'] = $com_code;
                    //  return $data_insert;
                    $flag = insert(new SuppliersWithOrdersDetails(), $data_insert);
                    if ($flag) {
                        /** update parent pill */
                        $total_details_sum = get_sum_where(new SuppliersWithOrdersDetails(), 'total_price', array("suppliers_with_orders_auto_serial" => $request->auto_serial_parent, 'order_type' => 1, 'com_code' => $com_code));
                        ...
                        update(new SuppliersWithOrders(), $dataUpdateParent, array("auto_serial" => $request->auto_serial_parent, "com_code" => $com_code, 'order_type' => 1));
    ...
    public function reloadItemsDetails(Request $request)
    {
        if ($request->ajax()) {
            $com_code = auth()->user()->com_code;
            $auto_serial = $request->auto_serial_parent;
            $data = get_cols_where_row(new SuppliersWithOrders(), array("is_approved", "id"), array("auto_serial" => $auto_serial, "com_code" => $com_code, 'order_type' => 1));
            if (!empty($data)) {
                $details = get_cols_where(new SuppliersWithOrdersDetails(), array("*"), array('suppliers_with_orders_auto_serial' => $auto_serial, 'order_type' => 1, 'com_code' => $com_code), 'id', 'DESC');
                if (!empty($details)) {
                    foreach ($details as $info) {
                        $info->item_card_name = InventoryItemCart::where('item_code', $info->item_code)->value('name');
                        $info->uom_name = get_field_value(new InvUoms(), "name", array("id" => $info->uom_id));
                ...
                // echo json_encode("Done Reload..!");
            }
            return view("admin.suppliers_with_orders.reload_items_details", ['data' => $data, 'details' => $details]);
        }
    }
    public function reloadParentPill(Request $request)
    {
        if ($request->ajax()) {
            $com_code = auth()->user()->com_code;
            $data = get_cols_where_row(new SuppliersWithOrders(), array("*"), array("auto_serial" => $request->auto_serial_parent, "com_code" => $com_code, 'order_type' => 1));
            if (!empty($data)) {
                $data['added_by_admin'] = Admin::where('id', $data['added_by'])->value('name');
                $data['supplier_name'] = Suppliers::where('supplier_code', $data['supplier_code'])->value('name');
                $data['store_name'] = Store::where('id', $data['store_id'])->value('name');
    ...
    public function loadEditItemDetails(Request $request)
    {
        if ($request->ajax()) {
            $com_code = auth()->user()->com_code;
            $parent_pill_data = get_cols_where_row(new SuppliersWithOrders(), array("is_approved"), array("auto_serial" => $request->auto_serial_parent, "com_code" => $com_code, 'order_type' => 1));
            if (!empty($parent_pill_data)) {
                if ($parent_pill_data['is_approved'] == 0) {
                    $item_data_details = get_cols_where_row(new SuppliersWithOrdersDetails(), array("*"), array("suppliers_with_orders_auto_serial" => $request->auto_serial_parent, "com_code" => $com_code, 'order_type' => 1, 'id' => $request->id));
                    $item_cards = get_cols_where(new InventoryItemCart(), array("name", "item_code", "item_type"), array('active' => 0, 'com_code' => $com_code), 'id', 'DESC');
                    $item_card_Data = get_cols_where_row(new InventoryItemCart(), array("does_has_retail_unit", "retail_uom_id", "uom_id"), array("item_code" => $item_data_details['item_code'], "com_code" => $com_code));
                    if (!empty($item_card_Data)) {
                        if ($item_card_Data['does_has_retail_unit'] == 1) {
                            $item_card_Data['parent_uom_name'] = get_field_value(new InvUoms(), "name", array("id" => $item_card_Data['uom_id']));
                            $item_card_Data['retail_uom_name'] = get_field_value(new InvUoms(), "name", array("id" => $item_card_Data['retail_uom_id']));
    ...
    public function loadModelAndDetails(Request $request)
    {
        if ($request->ajax()) {
            $com_code = auth()->user()->com_code;
            $parent_pill_data = get_cols_where_row(new SuppliersWithOrders(), array("is_approved"), array("auto_serial" => $request->auto_serial_parent, "com_code" => $com_code, 'order_type' => 1));
            if (!empty($parent_pill_data)) {
                if ($parent_pill_data['is_approved'] == 0) {
                    $item_cards = get_cols_where(new InventoryItemCart(), array("name", "item_code", "item_type"), array('active' => 1, 'com_code' => $com_code), 'id', 'DESC');
                    return view("admin.suppliers_with_orders.load_add_new_itemdetails", ['parent_pill_data' => $parent_pill_data, 'item_cards' => $item_cards]);
                }
            }
        }
    }
    public function editItemDetails(Request $request)
    {
        // return $request;
        $com_code = auth()->user()->com_code;
        $parent_pill_data = get_cols_where_row(new SuppliersWithOrders(), array(
            "is_approved", "order_date", "tax_value",
            "discount_value"
        ), array("auto_serial" => $request->auto_serial_parent, "com_code" => $com_code, 'order_type' => 1));

        // return  $parent_pill_data;
        if ($request->ajax()) {
            $com_code = auth()->user()->com_code;
            $parent_pill_data = get_cols_where_row(new SuppliersWithOrders(), array(
                "is_approved", "order_date", "tax_value",
            ...
            if (!empty($parent_pill_data)) {
                if ($parent_pill_data['is_approved'] == 0) {
                    ...
                    $data_to_update['is_parent_uom'] = $request->is_parent_uom;
                    if ($request->type == 2) {
                    ...
                    $flag = update(new SuppliersWithOrdersDetails(), $data_to_update, array("id" => $request->id, 'com_code' => $com_code, 'order_type' => 1, 'suppliers_with_orders_auto_serial' => $request->auto_serial_parent));
                    if ($flag) {
                        /** update parent pill */
                        $total_details_sum = get_sum_where(new SuppliersWithOrdersDetails(), 'total_price', array("suppliers_with_orders_auto_serial" => $request->auto_serial_parent, 'order_type' => 1, 'com_code' => $com_code));
                        $dataUpdateParent['total_cost_items'] = $total_details_sum;
    ...
    public function delete($id)
    {
        try {
            $com_code = auth()->user()->com_code;
            $parent_pill_data = get_cols_where_row(new SuppliersWithOrders(), array("is_approved", "auto_serial"), array("id" => $id, "com_code" => $com_code, 'order_type' => 1));
            if (empty($parent_pill_data)) {
                return redirect()->back()
            ...
            $flag = delete(new SuppliersWithOrders(), array("id" => $id, "com_code" => $com_code, 'order_type' => 1));
            if ($flag) {
                delete(new SuppliersWithOrdersDetails(), array("suppliers_with_orders_auto_serial" => $parent_pill_data['auto_serial'], "com_code" => $com_code, 'order_type' => 1));
    ...
    public function delete_details($id, $parent_id)
    {
        try {
            $com_code = auth()->user()->com_code;
            $parent_pill_data = get_cols_where_row(new SuppliersWithOrders(), array("is_approved", "auto_serial"), array("id" => $parent_id, "com_code" => $com_code, 'order_type' => 1));
            if (empty($parent_pill_data)) {
                return redirect()->back()
                    ->with(['error' => 'عفوا حدث خطأ ما']);
            }
            ...
            $item_row = SuppliersWithOrdersDetails::find($id);
            if (!empty($item_row)) {
                $flag = $item_row->delete();
                if ($flag) {
                    /** update parent pill */
                    $total_details_sum = get_sum_where(new SuppliersWithOrdersDetails(), 'total_price', array("suppliers_with_orders_auto_serial" => $parent_pill_data['auto_serial'], 'order_type' => 1, 'com_code' => $com_code));
                    $dataUpdateParent['total_cost_items'] = $total_details_sum;
                    $dataUpdateParent['total_before_discount'] = $total_details_sum + $parent_pill_data['tax_value'];
                    $dataUpdateParent['total_cost'] = $dataUpdateParent['total_before_discount'] - $parent_pill_data['discount_value'];
                    $dataUpdateParent['updated_by'] = auth()->user()->id;
                    $dataUpdateParent['updated_at'] = date("Y-m-d H:i:s");
                    update(new SuppliersWithOrders(), $dataUpdateParent, array("id" => $parent_id, "com_code" => $com_code, 'order_type' => 1));
                    return redirect()->back()
    ...
    public function load_modal_approve_invoice(Request $request)
    {
        if ($request->ajax()) {
            ...
            // //current user shift
            // $user_shift = get_user_shift(new AdminsShifts(), new Treasuries(), new Treasuriestransactions());
            $counterDetails = get_count_where(
                new SuppliersWithOrdersDetails(),
                array(
                    "suppliers_with_orders_auto_serial" => $request->auto_serial_parent,
                    "com_code" => $com_code, 'order_type' => 1
                )
            );
            return view("admin.suppliers_with_orders.load_modal_approve_invoice", ['data' => $data, 'counterDetails' => $counterDetails]);
        }
    }
    ...
    function do_approve($auto_serial, Request $request)
    {
        ...
        $data = get_cols_where_row(new SuppliersWithOrders(), array("total_cost_items", "is_approved", "id", "account_number", "store_id", "supplier_code"), array("auto_serial" => $auto_serial, "com_code" => $com_code, 'order_type' => 1));
        if (empty($data)) {
            return redirect()->route("admin.suppliers_orders.index")->with(['error' => "عفوا غير قادر علي الوصول الي البيانات المطلوبة !!"]);
        }
        $SupplierName = get_field_value(new Suppliers(), "name", array("com_code" => $com_code, "supplier_code" => $data['supplier_code']));
        if ($data['is_approved'] == 1) {
            return redirect()->route("admin.suppliers_orders.show", $data['id'])->with(['error' => "عفوا لايمكن اعتماد فاتورة معتمده من قبل !!"]);
        }
        $counterDetails = get_count_where(new SuppliersWithOrdersDetails(), array("suppliers_with_orders_auto_serial" => $auto_serial, "com_code" => $com_code, 'order_type' => 1));
        if ($counterDetails == 0) {
        ...
        $flag = update(new SuppliersWithOrder(), $dataUpdateParent, array("auto_serial" => $auto_serial, "com_code" => $com_code, 'order_type' => 1));
        if ($flag) {
            ...
            refresh_account_blance_Suppliers($data['account_number'], new Account(), new Suppliers(), new Treasuries_transactions(), new SuppliersWithOrder(), new services_with_orders(), false);
            //store move حركة المخزن
            //first Get item card data جنجيب الاصناف اللي علي الفاتورة
            $items = get_cols_where(new SuppliersWithOrdersDetails(), array("*"), array("suppliers_with_orders_auto_serial" => $auto_serial, "com_code" => $com_code, "order_type" => 1), "id", "ASC");
            if (!empty($items)) {
                foreach ($items as $info) {
                    //get itemCard Data
                    $itemCard_Data = get_cols_where_row(new InventoryItemCart(), array("uom_id", "retail_uom_quntToParent", "retail_uom_id", "does_has_retail_unit"), array("com_code" => $com_code, "item_code" => $info->item_code));
                    if (!empty($itemCard_Data)) {
                        ...
                        if ($info->is_parent_uom == 1) {
                            $quntity = $info->delivered_quantity;
                            ...
                            $quntity = ($info->delivered_quantity / $itemCard_Data['retail_uom_quntToParent']);
                    ...
                    if ($info->is_parent_uom == 1) {
                        ...
                        if ($itemCard_Data['does_has_retail_unit'] == 1) {
                    ...
                    update(new InventoryItemCart(), $dataUpdateItemCardCosts, array("com_code" => $com_code, "item_code" => $info->item_code));
                    // update itemcard Quantity mirror  تحديث المرآه الرئيسية للصنف
                    do_update_itemCardQuantity(new InventoryItemCart(), $info->item_code, new Inv_itemcard_batches(), $itemCard_Data['does_has_retail_unit'], $itemCard_Data['retail_uom_quntToParent']);
                }
            }
    ...
    public function ajax_search(Request $request)
    {
        if ($request->ajax()) {
            ...
            $data = SuppliersWithOrders::where($field1, $operator1, $value1)->where($field2, $operator2, $value2)->where($field3, $operator3, $value3)->where($field4, $operator4, $value4)->where($field5, $operator5, $value5)->where('order_type', '=', 1)->orderBy('id', 'DESC')->paginate(PAGINATION_COUNT);
            if (!empty($data)) {
                foreach ($data as $info) {
                    $info->added_by_admin = Admin::where('id', $info->added_by)->value('name');
                    $info->supplier_name = Suppliers::where('supplier_code', $info->supplier_code)->value('name');
    ...
    public function printsaleswina4($id, $size)
    {
        ...
        try {
            $com_code = auth()->user()->com_code;
            $invoice_data = get_cols_where_row(new SuppliersWithOrder(), array("*"), array("id" => $id, "com_code" => $com_code, 'order_type' => 1));
            if (empty($invoice_data)) {
                return redirect()->route('admin.suppliers_orders.index')->with(['error' => 'عفوا غير قادر علي الوصول الي البيانات المطلوبة !!']);
            }
            $invoice_data['added_by_admin'] = Admin::where('id', $invoice_data['added_by'])->value('name');
            $invoice_data['supplier_name'] = Suppliers::where('supplier_code', $invoice_data['supplier_code'])->value('name');
            $invoice_data['supplier_phone'] = Suppliers::where('supplier_code', $invoice_data['supplier_code'])->value('phones');

            $invoice_data['store_name'] = Store::where('id', $invoice_data['store_id'])->value('name');

            $invoices_details = get_cols_where(new SuppliersWithOrdersDetails(), array("*"), array('suppliers_with_orders_auto_serial' => $invoice_data['auto_serial'], 'order_type' => 1, 'com_code' => $com_code), 'id', 'DESC');
