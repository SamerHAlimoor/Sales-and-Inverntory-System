...
class InventoryUomController extends Controller
{
    ...
    {
        $data = InvUoms::select()->orderby('id', 'DESC')->paginate(PAGINATION_COUNT);
        if (!empty($data)) {
    ...
    public function store(InvUomsRequest $request)
    {
        try {
            ...
            //check if not exsits
            $checkExists = InvUoms::where(['name' => $request->name, 'com_code' => $com_code])->first();
            if ($checkExists == null) {
    ...
    public function edit($id)
    {
        ...
        $total_counter_used = 0;
        // $sales_invoices_detailsCount = 0;
        return view('admin.inv_uom.edit', [
            'data' => $data,

            'total_counter_used' => $total_counter_used
        ]);
    }
    public function update($id, InvUomUpdateRequest $request)
    {
        try {
            ...
            $checkExists = InvUoms::where(['name' => $request->name, 'com_code' => $com_code])->where('id', '!=', $id)->first();
            if ($checkExists != null) {
                return redirect()->back()
                    ->with(['error' => 'عفوا اسم الوحدة مسجل من قبل'])
                    ->withInput();
            ...
            InvUoms::where(['id' => $id, 'com_code' => $com_code])->update($data_to_update);
    ...
    public function ajax_search(Request $request)
    {
        if ($request->ajax()) {
            ...
            $data = InvUoms::where($field1, $operator1, $value1)->where($field2, $operator2, $value2)->orderBy('id', 'DESC')->paginate(PAGINATION_COUNT);
            if (!empty($data)) {
                foreach ($data as $info) {
                    $info->added_by_admin = Admin::where('id', $info->added_by)->value('name');
                    if ($info->updated_by > 0 and $info->updated_by != null) {
