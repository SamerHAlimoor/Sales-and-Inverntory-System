...
@section('content')
      ...
      متزن
          ...
          @endif
            ...
            <td @if($info->active==0) class="bg-success" @else class="bg-danger" @endif  >@if($info->active==0) مفعل @else معطل @endif</td> 
      
         <td>
      @if( $info->relatediternalaccounts==0)
        <a href="{{ route('admin.accounts.edit',$info->id) }}" class="btn btn-sm  btn-primary">تعديل</a>  
        <form action="{{ route('admin.accounts.delete',$info->id) }}" method="post" style=" display: inline-block;">
